...
export const insertOrganisation = async() => {
    ...
    const result = await db.insert(organisation).values(newOrganisation).returning();
    if(result[0].id){
...
export const removeOrganisation = async(name: string) => {
    const deleted = await db.delete(organisation).where(eq(organisation.name, name)).returning({deleted: organisation.id});
