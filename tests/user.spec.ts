...
import { injectOrganisation, removeOrganisation, removeUser } from "./functions";
...
test('the link named "Créer" should redirect to add-user page', async ({
  ...
  await page.getByTestId("organisation-select").selectOption({ label: organisation.name });
  ...
  await page.getByRole('link', { name: 'Accéder à la page dashboard/utilisateur Utilisateurs' }).click();
  await expect(page.getByRole('cell', { name: 'Test', exact: true })).toContainText('Test');
  await expect(page.getByRole('cell', { name: 'test@access-collect.fr' })).toContainText('test@access-collect.fr');
  await expect(page.getByRole('cell', { name: 'admin' })).toContainText('admin');
  await expect(page.getByRole('cell', { name: 'Organisation-test-add-user' })).toContainText('Organisation-test-add-user');
